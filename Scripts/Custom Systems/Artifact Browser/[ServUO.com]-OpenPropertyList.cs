...
namespace Server
{
    ...
    public sealed class AllPropertyList
    {
        ...
        public OpenPropertyList GetOpenProperties(BaseWeapon weapon)
        {
            ...
            if (wielder != null)
            {
                if (Server.Spells.Mystic.EnchantSpell.IsUnderSpellEffects(wielder, weapon))
                {
                    bonus = Server.Spells.Mystic.EnchantSpell.BonusAttribute(wielder);
                    enchantBonus = Server.Spells.Mystic.EnchantSpell.BonusValue(wielder);
                    fcMalus = Server.Spells.Mystic.EnchantSpell.CastingMalus(wielder, weapon);
                }
            }
            #endregion
			...
			if (AosCustomAttribute.HitFlamestrike > 0)
            {
                list.Add(1060423, ("Hit Flamestrike 50%").ToString()); // hit fireball ~1_val~%
            }
			if ((fprop = (double)weapon.CustomAttributes.HitChainLightning * focusBonus) != 0)
            {
                list.Add(1060423, ("Hit Chain Lightning 50%").ToString()); // hit lightning ~1_val~%
            }
			if ((fprop = (double)weapon.CustomAttributes.HitPoisonStrike * focusBonus) != 0)
            {
				list.Add(1060423, ("Hit PoisonStrike 50%").ToString()); // hit fireball ~1_val~%   
			}
			if ((fprop = (double)weapon.CustomAttributes.HitHailstorm * focusBonus) != 0)
            {
				list.Add(1060423, ("Hit Hailstorm 50%").ToString()); // hit fireball ~1_val~%     
			}
			if ((fprop = (double)weapon.CustomAttributes.HitArrowStorm * focusBonus) != 0)
            {
				list.Add(1060423, ("Hit ArrowStorm 50%").ToString()); // hit fireball ~1_val~%        
			}
			if ((fprop = (double)weapon.CustomAttributes.HitColdStrike * focusBonus) != 0)
            {
				list.Add(1060423, ("Hit Coldstrike 50%").ToString()); // hit fireball ~1_val~%         
			}
            ...
            if (ImmolatingWeaponSpell.IsImmolating(weapon))
            {
        ...
        public OpenPropertyList GetOpenProperties(BaseQuiver quiver)
        {
            ...
            if (quiver.Quality == ClothingQuality.Exceptional)
        ...
        public OpenPropertyList GetOpenProperties(BaseTalisman talisman)
        {
            ...
            if (talisman.ExceptionalBonus != 0)
                list.Add(1072395, "#{0}\t{1}", AosSkillBonuses.GetLabel(talisman.Skill), talisman.ExceptionalBonus); // ~1_NAME~ Exceptional Bonus: ~2_val~%

            if (talisman.SuccessBonus != 0)
                list.Add(1072394, "#{0}\t{1}", AosSkillBonuses.GetLabel(talisman.Skill), talisman.SuccessBonus); // ~1_NAME~ Bonus: ~2_val~%
        ...
        public OpenPropertyList GetOpenProperties(BaseJewel jewel)
        {
            ...
            #region Mondain's Legacy
            if (jewel.Quality == ArmorQuality.Exceptional)
                list.Add(1063341); // exceptional
