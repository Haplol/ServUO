...
namespace Server.Spells.Bard
{
    ...
    public static class BardHelper
    {
        ...
        {
            return Scaler(mobile, null, low, high, complementryModifier, complementrySkillsOnly);
        }

        public static int Scaler(Mobile mobile, Mobile target, int low, int high, double complementryModifier, bool complementrySkillsOnly = true)
        {
            ...
            {
                if (target != null)
                    scale *= GetSlayerDamageScalar(mobile, target);
            }
        ...
        public static double GetSlayerDamageScalar(Mobile caster, Mobile defender)
        {
            double scalar = 1.0;

            if (caster.Backpack != null)
            {
                BaseInstrument inst = (BaseInstrument)caster.Backpack.FindItemByType(typeof(BaseInstrument));                

                if (inst != null)
                {
                    SlayerEntry atkSlayer = SlayerGroup.GetEntryByName(inst.Slayer);
                    SlayerEntry atkSlayer2 = SlayerGroup.GetEntryByName(inst.Slayer2);
            ...
            if (defender.Backpack != null)
            {
                ISlayer defISlayer = (BaseInstrument)defender.Backpack.FindItemByType(typeof(BaseInstrument));
