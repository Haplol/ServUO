		...
		public class InternalSecondTarget : Target
		{
			...
			protected override void OnTarget(Mobile from, object targeted)
			{
				...
				if (targeted is BaseCreature || (from is BaseCreature && ((BaseCreature)from).CanProvoke))
				{
					...
					bool questTargets = QuestHelper.CheckIncite(from, m_Creature, creature, false);
					if (!m_Instrument.IsChildOf(from.Backpack))
					{
						from.SendLocalizedMessage(1062488); // The instrument you are trying to play is no longer in your backpack!
					}
					...
					{
						if (ItemExtensions.HasEquipped(from, typeof(SongWovenMantleplus))){
						...
						if (from.CanBeHarmful(m_Creature, true) && from.CanBeHarmful(target, true))
						{
							...
							else
							{
								...
								else
								{
									...
									if ( from is PlayerMobile && m_Creature is BaseCreature && creature is BaseCreature )
										QuestHelper.CheckIncite(from, m_Creature, creature);
